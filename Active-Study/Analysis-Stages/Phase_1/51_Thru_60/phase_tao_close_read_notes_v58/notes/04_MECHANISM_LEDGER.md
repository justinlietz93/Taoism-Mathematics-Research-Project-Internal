...
## v56 mechanism ledger addition — claim dependency graph

Mechanism spine now has explicit claim dependencies C01-C10. The spine is: retained carrier -> state-completeness gate -> corrected custody -> B scope -> modular-B candidate -> L/carry/re-chart -> external event/physical bridges -> Shadow external reference -> available-source terminal boundary.

This prevents semantic drift: Ancient Yi base-8 carry is retained as a modular-B candidate, not an exact B proof; corrected Orthad remains custody-first/readout-second; Shadow remains external reference.


## v57 final operational kernel

Retained carrier/state -> admissible operation -> carrier-specific refinement/update arithmetic -> completion/blockage/threshold -> carry/lift/re-chart -> boundary readout -> scalar/external comparison last. Projection is admissible only if state-complete for the next transition.
