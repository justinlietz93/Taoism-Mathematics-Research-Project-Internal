...
## Pass 35 mechanism additions

### Ancient Yi carrier/readout
retained line carrier -> binary/octal/decimal/base64/computer-counting readouts. Strong Orthad custody/readout analogue. Missing: legal successor/carry/floor rule.
### Tao Te Ching control
nonforcing/origin/return/name-vs-source conceptual guardrail. Useful for preventing premature projection identity, not direct QBL procedure evidence.


## Pass 36 mechanism locks

```text
Chinese logic: shi/ming/fa/lei/xiao/pi/yuan/tui = object/name/standard/kind/reasoning-operation grammar.
Finite automata: finite configuration -> enabled event -> transition -> run word -> accepting boundary.
Dynamic logic: state-transition graph separates single realized run from branching possible runs.
Category theory: object/morphism/composition/functor/projection gives rigorous cross-map language for normalization.
Ancient Yi: same retained line carrier supports binary/octal/decimal/computer-counting projections.
```

## ICPR / finite-memory / Chinese logic mechanism locks

### Finite-memory automata normal form

```text
finite registers
-> finite configuration
-> enabled action/event
-> transition/update
-> run word
-> accepting boundary
```
...
```text
Ξ̂ retained state
-> admitted Q/B/L operation
-> primitive update
-> accrued word
-> boundary readout/lift
```

### Chinese logic bridge-check terms

```text
fa  : standard/model/admissibility criterion
lei : kind, sameness/difference class
xiao: apply standard to case
pi  : explain target by known analogue
yuan: adduce accepted same-kind case
tui : transfer accepted sameness into disputed target
```

Use this as rigor tooling: bridge only when functional kind is preserved.

---

## Liu 2022 mechanism refinement — pass 38

| Mechanism | External instance | Phase-normalized bridge | Status |
|---|---|---|---|
| Retained state before projection | 3D MHD state evolved before synthetic AIA images | compute in lift, project/read at boundary | STRONG_CONFIRMED |
| Q-like oriented transfer | twist/shear/footpoint migration of MFR | oriented phase/topology transport | BRIDGE_OPEN |
| B-like refinement | HFT/current-sheet/QSL stress concentration | state-dependent narrowing/refinement until topology cannot continue | BRIDGE_OPEN |
| L-like promotion | high sigmoid separates/erupts while low filament remains | re-articulation + promoted branch + retained remainder | BRIDGE_OPEN_HIGH_VALUE |
| Orthad readout | high-Q ribbons, 94/304 synthetic images | support-channel / terminal projection from retained state | STRONG_BRIDGE |
| J+M split | ideal instability + reconnection/heating/conduction | conservative/dissipative channel split candidate | EQUATION_LEVEL_BRIDGE_OPEN |
