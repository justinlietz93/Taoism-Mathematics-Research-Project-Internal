...
## Pass 19 mechanism additions

### Modular cocycle / Shadow preparation

```text
Source: Modular-Forms/MFLecture1.pdf
Mechanism:
  SL2 acts on H.
  PSL2 quotient collapses ±I in visible action.
  j(g,τ)=cτ+d is retained automorphy data.
  j(gg',τ)=j(g,g'τ)j(g',τ) is cocycle composition.
Read:
  Shadow comparison must preserve transformation/cocycle data before scalar q-series projection.
```

### Yin-Yang computational L lineage

```text
Sources:
  Yin-Yang_grid_and_geodynamo_simulation.pdf
  1003.1633v1.pdf
  Geochem Geophys Geosyst 2014 Yoshida visualization paper
Mechanism:
  single spherical chart has pole/symmetry-axis failure;
  two complementary low-latitude patches avoid the singular domain;
  overlap + interpolation supplies boundary continuation;
  conservation/visualization/projection depend on retained two-chart coherence.
Read:
  L = legal host re-chart, not decorative dimensional metaphor.
```

### Stale selector caution sharpened

```text
Source: Phase_Calculus_Complete_Formalisation.pdf
Use:
  Q/B/L primitive meanings;
  B(u,v)=sort(v,u+v);
  retained-state/projection-loss discipline.
Do not use:
  stale fixed W,Δ macro selector as canonical floor law.
```

---

## Pass 20 — Zagier modular/theta squeeze-dry
...
```text
SL2 action on H
PSL2 quotient collapse
modular transformation law with automorphy factor (cz+d)^k
finite-dimensional modular-form spaces
q-expansion as cusp projection
E2 anomaly / quasimodular correction
eta product q^(1/24)Π(1-q^n)
eta unary-theta representation Σχ12(n)q^(n²/24)
Serre-Stark: weight 1/2 forms are linear combinations of unary theta series
Rankin-Cohen / corrected derivative structures
CM/lacunary support examples
```

Q/B/L/Shadow mapping:

```text
Q: weak/moderate via group generators and half-weight multipliers.
B: indirect via admissible cusp/support/product constraints.
L: strong via re-chart action, automorphy factor, S inversion, quotient loss.
Shadow: very strong via eta/unary theta/χ12/q^(n²/24).
```

New work order:

```text
Test Shadow as Jacobi-variable derivative:
Θ12(ζ,τ)=Σχ12(n)ζ^nq^(n²/24)
ζ∂ζΘ12|ζ=1 = Σχ12(n)nq^(n²/24)
```
