# Mechanism Ledger

This ledger is for stable mappings only. If a mapping requires changing definitions, move it to contradictions/open questions.

## Q: six-seat carrier / four-phase orbit

### Internal definition

```text
six seats = {+x, -x, +y, -y, +z, -z}
Q orbit = four equatorial seats by quarter-turn
axis/radix poles = two fixed/mediating seats
```

### External leads

| Source | External structure | Status | Notes |
|---|---|---|---|
| Shape of Understanding | 4 appearances + 2 builder states = 6-level reading | strong lead | Same 4-visible + 2-generator pattern, but not yet numeric Q. |
| 5d565 DDC math | Dao as pole/reference point, De as radius, DDC as circle | strong lead | Radix/radius/circle framework. |
| Chinese four-elements algebra / Britannica lead | center + four directions | queued | Need close-read actual included sources. |
| Taijitu / Yin-Yang geometry | circle, S-boundary, complementary phases | medium lead | Morphological, not enough by itself. |

## B: depth-9 Fibonacci/Farey corridor

### Internal definition

```text
B(u,v)=sort(v,u+v)
B^9(1,1)=(55,89)
uv=4895
r=1/4895
```

### External leads

| Source | External structure | Status | Notes |
|---|---|---|---|
| Shape of Understanding | repeated addition of Yin/Yang pair, binary tree | structural lead | Branch refinement exists; no floor-anchor proof yet. |
| Farey/Stern-Brocot/modular material | mediants / continued fractions / Farey graph | expected strongest | Need full source pass. |
| Chinese element methods | one/two/three/four unknown expansion | open | Check if expansion has path-depth/floor behavior. |

## L: lift / orthogonal re-articulation / re-chart

### Internal definition

```text
same-axis saturation/failure
→ orthogonal re-articulation
→ new carrier/chart/domain
→ carried state preserved while projection changes
```

### External leads

| Source | External structure | Status | Notes |
|---|---|---|---|
| Tao of Math | imaginary numbers on perpendicular axis; complex plane | exact mechanism-level lead | Blocked real-axis operation legalized by orthogonal axis. |
| Shape of Understanding | Pre-Heaven to Post-Heaven by middle-line inversion and center split | strong lead | Internal line-state change changes visible identity and arrangement. |
| Modular forms | SL2 lift vs PSL2 projection, S inversion | strong lead | Sign/phase retained in lift, collapsed in quotient. |
| Yin-Yang grid | coordinate singularity avoided by complementary overlapping chart | strong engineering lead | Re-charting instead of forcing one global coordinate frame. |

## Projection / witness loss

### Internal definition

Visible witness is not necessarily state-complete. Carried history/branch/lift may be erased by projection.

### External leads

| Source | External structure | Status | Notes |
|---|---|---|---|
| Modular forms | ± matrices act the same in PSL but differ in SL | strong lead | Exact quotient/projection-loss analogue. |
| Yijing | same primitives, different ordering, different hexagram meaning | medium lead | Needs frozen rule extraction. |
| Yin-Yang grid | same sphere, different coordinate patches/readouts | medium lead | Need figure pass. |

## Shadow / residual

### Internal definition

q-series / character-like residual with exponent/sign/support pattern.

### External leads

| Source | External structure | Status | Notes |
|---|---|---|---|
| Zagier modular forms | theta series, characters, eta-like q-series | strong lead | Need transform behavior. |
| Dedekind eta / eta powers | q^(1/24), product/series expansions | strong lead | Need coefficient and S/T transformation comparison. |


## Pass 17 additions

### B/Q selector analogue: Linggui Bafa

Source: `Prospect-Leads/philosophies-10-00111.pdf`

```text
yinyang day-state -> choose divisor 9 or 6
stem-branch sum   -> remainder
remainder         -> jiugong position
position          -> acupuncture point/action
remainder 5       -> exceptional branch via male/female rule
```

Status: **strong external selector lead**. This is not proof of Phase Calculus, but it is an exact state-conditioned operation grammar of the kind the B/Q floor-anchor prospect requires.

### Q/regime threshold analogue: Taijitu Zurich model

Source: `Tao-Research/out/2606.18009v1.pdf`

```text
hidden continuous change
-> visible stable/metastable regime persists
-> critical threshold crossed
-> visible dominance changes
```

Status: **strong Q-capacity/regime lead**. Supports hidden B activity under visible Q stability.

### Yijing negative-control lock

Source: `Tao-Research/out/ChinaandUniversals.pdf`

Do not reduce hexagrams to ordinary open binary arithmetic. Treat them as a closed six-line state carrier with orientation/readout ambiguity and native rule-dependence.

### Shadow exact target lock

Source: `Modular-Forms/Zagier123ModularForms.pdf`

```text
eta(z) = sum chi_12(n) q^(n^2/24)
chi_12(12m ± 1)=1
chi_12(12m ± 5)=-1
chi_12(n)=0 if 2|n or 3|n
```

Status: **exact Shadow-channel target**.

## Pass 18 additions

### Xiantian/Houtian as retained-carrier re-chart

Source: `Tao-Research/out/1705.0203v1.pdf`

```text
same Gua carrier
→ Xiantian tree/cube/equal-state organization
→ Houtian root/center/cycle/family-role organization
→ same elements but different order, position, pairing, and computation direction
```

Status: **strong L/readout/projection-loss lead**.

### Modular automorphy cocycle

Sources: `MFLecture1.pdf`, `Zagier123ModularForms.pdf`

```text
τ ↦ (aτ+b)/(cτ+d)
j(g,τ)=cτ+d
j(gg',τ)=j(g,g'τ)j(g',τ)
SL2 vs PSL2 quotient collapses sign/lift information
```

Status: **strong retained-channel / Shadow transformation lead**.

### Yin-Yang grid as hard computational L

Sources: `Axis-Free-overset-grid.pdf`, Yin-Yang visualization/MFE papers

```text
single spherical grid hits pole singularity
→ artificial boundary artifacts / time-step restriction
→ two overlapping low-latitude charts
→ patch transform + interpolation carries state
→ physical solution continues without singular-axis boundary law
```

Status: **very strong engineering analogue for L**.

### Non-collapsible dual operations

Source: `Physics_of_Buddhism_The_physics_and_math.pdf`

```text
* and ⋆ are distinct involutions / dual operations.
not(not X) may not be ordinary double negation if the two NOTs are different operators.
```

Status: **control rule for Yijing/Phase mapping**: do not collapse line complement, order reversal, trigram exchange, and re-chart into one operation.
