...
def run(root:Path)->dict[str,Any]:
    ...
    structure={
      ...
      "soficity_argument":["refinement mesh tends to zero","boundary-adjacent cylinder follower arcs have common endpoint D^n(p)","follower language determines an open arc","finite follower sets would force D^n(p)=D^m(p)","p irrational excludes eventual periodicity"],
      "mixing":"PROVED TOPOLOGICALLY MIXING",
      ...
      "finite_markov_order":"NO FINITE ORDER; follows from non-soficity; exact order 1..10 witnesses included",
    ...
    summary={"step":"p5-b1-v6","direct_cylinders_depth12":sum(len(x) for x in levels),"length3_realizable":15,"complexity_n20":2**21-1,"soficity":"PROVED NON-SOFIC","mixing":"PROVED","boundary_min":boundary["minimum_boundary_distance_lower_bound"],"empirical_vs_J":comparison["metrics_empirical_vs_J"],"empirical_vs_K":comparison["metrics_empirical_vs_K_edge_envelope"]}
