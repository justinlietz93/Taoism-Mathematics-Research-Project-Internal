# QBL Carry Affine Follower Structure v5

**Step:** `p5-b1-v7`  
**Primary authority:** `QBL_PRIMITIVE_CUSTODY_AND_ORTHAD_LAW_v2.md`  
**Scope:** affine ceiling carry language, standard half-open follower sets, exact interval coding, complexity, entropy, non-soficity, and mixing.  
...
- The standard follower sets of the actual half-open word cylinders are separated by their half-open follower regions.
- The affine carry language is **not sofic**. No finite labeled-graph presentation exists.
- The affine carry language is **topologically mixing**, by a proof independent of non-soficity.
- Consequently, the language has no finite Markov order.
...
The interval coding itself is a complete presentation of the affine language. This pass closes the missing bridge from standard follower sets of half-open cylinders to the follower-region argument and proves that no **finite-state/sofic** presentation can be equivalent to it.
...
## 10. Standard follower sets and non-soficity

This section repairs the endpoint bridge left open in `p5-b1-v6`. All cylinders and follower regions retain the actual half-open convention. No continuation is inferred by replacing a half-open set with its interior.

### 10.1 Standard follower sets and the exact half-open identity

For a realizable word

\[
w=w_0w_1\cdots w_{n-1},
\]

define its actual word cylinder by

\[
C(w)=\bigcap_{j=0}^{n-1}D^{-j}(A_{w_j}),
\]

where each base atom `A_7,A_8,A_9` has the half-open endpoint assignment fixed in Section 6. Define the standard follower set

\[
\operatorname{Fol}(w)=\{v:wv\in\mathcal L\}
\]

and the actual half-open follower region

\[
H_w=D^n(C(w)).
\]

For every finite word `v`, concatenation gives the exact set identity

\[
\boxed{C(wv)=C(w)\cap D^{-n}(C(v)).}
\]

Both sides impose the same itinerary conditions: the first `n` coordinates must lie in the atoms named by `w`, and the subsequent coordinates must lie in the atoms named by `v`. Therefore

\[
\begin{aligned}
v\in\operatorname{Fol}(w)
&\Longleftrightarrow C(wv)\ne\varnothing\\
&\Longleftrightarrow \exists x\in C(w):D^n(x)\in C(v)\\
&\Longleftrightarrow H_w\cap C(v)\ne\varnothing.
\end{aligned}
\]
...
This is the exact relation needed for standard follower sets. It includes every endpoint case:

1. an interior source point followed by an interior future point;
2. an included source endpoint followed by an interior future point;
3. an interior source point followed by an included future endpoint;
4. an included source endpoint followed by an included future endpoint.

In every case the point is counted precisely when it belongs to both half-open sets in the displayed identity. An excluded endpoint is not silently restored by closure or by continuity.

### 10.2 What the follower language determines

For any half-open arc `H`, define

\[
\Phi(H)=\{v:H\cap C(v)\ne\varnothing\}.
\]

The shrinking-cylinder basis gives the following one-way separation lemma:

\[
\boxed{
\Phi(H)=\Phi(H')
\Longrightarrow
\operatorname{int}H=\operatorname{int}H'.}
\]

Proof. Suppose the interiors are different. Two distinct open arcs have a nonempty open subarc `U` contained in one and disjoint from the closure of the other. Choose a point of `U` outside the countable refinement-boundary set. Since the cylinder mesh tends to zero, a sufficiently deep half-open cylinder `C(v)` containing that point is wholly contained in `U`. Then `v` belongs to exactly one of `Phi(H)` and `Phi(H')`, a contradiction.

The implication deliberately stops at equality of interiors. Finite-word follower data need not distinguish two sets that differ only by an isolated endpoint. The non-soficity proof below does not require that stronger and generally unsafe statement.

### 10.3 Boundary-adjacent cylinders and exact half-open follower regions

Let

\[
\mathcal B_n=\bigcup_{k=0}^{n}D^{-k}(p)
\]

be the length-`n` refinement boundary. Let `ell_n` and `r_n` be the immediate boundary neighbors of `p` in positive circular order. The two actual length-`n` cylinders adjacent to `p` are
...
Let `g_n^-` and `g_n^+` be the consecutive points of `D^{-n}(p)` whose gap contains `p`. Irrationality gives
...
Because `ell_n,r_n` are refinement-boundary neighbors,

\[
g_n^-\le \ell_n<p<r_n\le g_n^+.
\]
...
Their union lies in the single half-open fundamental gap `(g_n^-,g_n^+]`. The map `D^n` is injective on that gap: two points with the same image differ by an integer multiple of `2^{-n}`, and the only possible pair at distance exactly `2^{-n}` would be the two gap endpoints, of which the left endpoint is excluded.
...
using the oriented lift inherited from the fundamental gap. Since `D^n` is increasing and injective there, the exact half-open follower regions are
...
\[
\operatorname{int}H_n^-=(\alpha_n,q_n),
\qquad
\operatorname{int}H_n^+=(q_n,\beta_n).
\]

The point `q_n=D^n(p)` is the uniquely oriented handoff from the minus interior to the plus interior: moving in positive circular order, the minus interior ends and the plus interior begins at `q_n`.

A stronger unqualified claim that the two closures always have only one common boundary is unnecessary. If the source pair fills the entire `D^{-n}(p)` fundamental gap, the two image arcs are complementary and their closures share a second endpoint. At that second endpoint the orientation is reversed, from plus to minus. The ordered interior pair therefore still identifies `q_n` uniquely.

### 10.4 Distinct standard follower-set pairs

Let `w_n^-` and `w_n^+` be the words carried by `C_n^-` and `C_n^+`. By Section 10.1,

\[
\operatorname{Fol}(w_n^\pm)=\Phi(H_n^\pm).
\]

Assume that for `n<m` the ordered standard follower-set pairs agree:

\[
\left(
\operatorname{Fol}(w_n^-),
\operatorname{Fol}(w_n^+)
\right)
=
\left(
\operatorname{Fol}(w_m^-),
\operatorname{Fol}(w_m^+)
\right).
\]

The separation lemma then gives

\[
\operatorname{int}H_n^-=\operatorname{int}H_m^- ,
\qquad
\operatorname{int}H_n^+=\operatorname{int}H_m^+ .
\]

Equal ordered interior pairs have the same oriented minus-to-plus handoff. Consequently
...
This makes `p` eventually periodic under doubling. Every eventually periodic binary point is rational, contradicting Section 7. Therefore the ordered standard follower-set pairs are all distinct:
...
### 10.5 Non-soficity and finite Markov order

In a finite labeled graph, the follower set of a word is determined by the finite subset of vertices at which a path labeled by that word can terminate. A sofic language therefore has only finitely many distinct word follower sets, and hence only finitely many ordered follower-set pairs.

Section 10.4 supplies infinitely many distinct ordered pairs. Thus
...
A language with finite Markov order is a shift of finite type, and every shift of finite type is sofic. Therefore

\[
\boxed{\text{the affine carry language has no finite Markov order}.}
\]
...
The order `1..10` rational-affine witnesses remain finite certificates. They are not used as the global proof.
...
The package contains Lean theorem surfaces for the `J` identities, the complete `989` prefix theorem, and the half-open follower bridge. The new follower file defines the actual `Ioc` convention and proves the exact nonemptiness equivalence between a concatenation cylinder and the intersection of a half-open follower region with a future cylinder. It also records the included/excluded handoff endpoint asymmetry.
