...
## B/Q coupled floor anchor refinement

### Correction

B must be modeled as capacity-gated by Q, not as a standalone Fibonacci counter.

```text
B priority refinement:
  q=(u,v) advances by B while admissible

Q-defined budget:
  Q occupied/visited positions determine current floor/capacity

B block:
  next B would exceed current capacity

Q move:
  quarter-turn expands/changes admitted budget

L trigger:
  B blocked and Q sign/position cycle saturated
```

### Current internal candidate formula

```text
floor_capacity(n) ≈ 2^(2*n)
```

where `n` is the number of Q positions occupied/visited. Exact formula pending latest source.

### First closure

```text
B/Q coupled walk eventually reaches:
  9 B steps
  6 Q positions/orientations
  then L
```

### External search rule

External B analogues must be capacity-gated refinement systems, not merely any nine-step sequence.

Priority signatures:

```text
refinement until floor/capacity
phase/orientation expands budget
finite position cycle
lift after joint saturation
```
